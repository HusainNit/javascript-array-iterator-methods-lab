...
inventors.filter((inventorObj)=>{
    (inventorObj.year >= 1500 && inventorObj.year <= 1599) && veryOldInventors.push(inventorObj);
})
...
inventors.map((inveObj)=>{
    inventorNames.push({first:`${inveObj.first}`, last:`${inveObj.last}`})
})
...
sortedByBirthYear.push(inventors.slice().sort((a,b)=>a.year - b.year)) // i use slice to not alter the original inventors array, slice will create copy of the array then it will be sorted by the year
...
inventorNamedAda=(inventors.find((invterObj)=> invterObj.first ==='Ada'))
...
people.forEach((nameWithcomma)=>{
    nameWithcomma.split(`\n`).forEach((name)=>{
        firstLast.push(name.split(`, `).reverse().join(" "))
    })
})
...
isAdultPresent=devs.some((devObj) =>  new Date().getFullYear()-devObj.year >=18)
...
isEveryone19OrOlder = devs.every((devObj)=> new Date().getFullYear()-devObj.year>=19)
// Check your work:
...
commentById = comments.find((comObj)=> comObj.id ===823423)
...
// Complete the exercise in the space below:
idx=comments.findIndex((comObj)=> comObj.id ===123523)
...
totalYearsLived = inventors.reduce((accumulate,current)=>{
    return accumulate+=(current.passed -current.year)
},0)
...
travelMethodCounts = travelMethods.reduce((accum,current)=>{
    if(accum[current])
        accum[current]+=1;
    else
    accum[current]=1
